...
if test $ver = snapshot; then
	...
	openwrt_url=https://hk2.codet.net/snapshots/targets/${arch}/${subarch}
else
	openwrt_branch=${dist}-${branch_ver}
	procd_url=https://github.com/openwrt/openwrt/branches/${openwrt_branch}/package/system/procd
	openwrt_url=https://hk2.codet.net/releases/${ver}/targets/${arch}/${subarch}
fi
...
build_tarball() {
	...
	$cmd $rootfs $opts -o $tarball --disable-services="sysfixtime sysntpd led" --arch=${arch} --subarch=${subarch} --version=${ver}
}
